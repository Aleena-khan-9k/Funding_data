...
export const searchFile = async (req, res) => {
  try {
    const { organization_name, website } = req.body; // Accept multiple search parameters

    if (!organization_name && !website) {
      return res.status(400).json({
        message: 'At least one search parameter (organization_name or website) is required',
      });
    }
